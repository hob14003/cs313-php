        ...
        if($_GET["characteristic"]) {
            ...
            if($_GET["characteristic"] == "All Characteristics") {
                ...
                echo "SELECT d.NAME, c.NAME, c.DESCRIPTION c.ID FROM DOMAIN d INNER JOIN CHARACTERISTIC c ON d.ID = c.DOMAIN_ID;";
                    ...
                    <tbody>";
                        foreach ($db->query("SELECT d.NAME, c.DESCRIPTION, c.ID FROM DOMAIN d INNER JOIN CHARACTERISTIC c ON d.ID = c.DOMAIN_ID WHERE c.NAME = " . "'" . $_GET["component"] . "';") as $row)
                        {
                            echo "<tr><td>" . $row[0] . "</td>
                            <td>" . $row[1] . "</td>
        ...
        if($_GET["domain"]) {
            echo "   
                ...
                <table class=\"table\">
                    ...
                    <tbody>";
            echo "<td>SELECT d.NAME, c.NAME, c.DESCRIPTION, c.ID FROM DOMAIN d INNER JOIN CHARACTERISTIC c ON d.ID = c.DOMAIN_ID WHERE D.NAME = '" . $_GET["domain"] . "';</td>";
            /*
