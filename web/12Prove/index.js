...
app.get('/home', function(req, res) {
    ...
    db.any('SELECT BOOK.ID, BOOK.BOOK_NUM, BOOK.NAME AS BOOK, BOOK.DESCRIPTION, SERIES.NAME AS SERIES, SERIES.SERIES_IMAGE AS SERIES_IMG, SERIES.COLOR AS COLOR FROM BOOK INNER JOIN SERIES ON BOOK.SERIES_ID=SERIES.ID WHERE SERIES.ID = ' + id + ' ORDER BY BOOK.ID', 4)
...
app.post('/comment', function(req, res) {
    var commentT = req.body.commentT;
    var nameT = req.body.nameT;
    console.log("comment: " + commentT);
    
    if (commentT === undefined) {       //if t=undefined, call tt
        console.log("undefined...")      //call t
    }
    else {
        console.log("comment: " + commentT)
    }
    ...
    db.none("INSERT INTO USR_COMMENT(NAME, COMMENT_TEXT) VALUES('" + nameT + "', '" + commentT + "')", 'A spren has removed this comment.')
    .then(() => {
        // success;
    ...
    .catch(error => {
        // error;
        alert("Any error occured that prevented insertion of the comment")
    });
    
    db.any('SELECT NAME, COMMENT_TEXT FROM USR_COMMENT', 4)

        .then(commentR => {
        console.log('COMMENTS:', commentR); //print data
        console.log({'comment': commentR}); //print data
        res.render('pages/sandersonBook', {'comment': commentR});
    })
        .catch(error => {
        console.log(error); // print the error;
    })
...
app.get('/book', function(req, res) {
    ...
    var db = pgp(cn); // database instance;
        console.log(id);
        db.any("SELECT ID AS BOOK_IMG, DESCRIPTION, NAME AS BOOK FROM BOOK WHERE DESCRIPTION = '" + id + "'", 'A')

        .then(bookInfo => {
        console.log('BOOK INFO:', bookInfo); //print data
        console.log({'bookInfo': bookInfo}); //print data
        res.render('pages/sandersonBook', {'bookInfo': bookInfo});
    })
        .catch(error => {
        console.log(error); // print the error;
    })
...
function search(req, res) {
	...
	var params = {s: searchString, apikey:"905dba91"};
